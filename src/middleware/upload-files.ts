  ...
  destination: (req: Request, _file, cb) => {
    try {
      ...
      if (!sender_id || !receiver_id)
        return cb(new Error('Missing sender_id or receiver_id'), undefined as unknown as string);
...
const chatImageUpload = multer({ storage: chatStorage, fileFilter: imageOnlyFilter }).array(
  'files',
  10
);
...
const reportUpload = multer({ storage: reportStorage, fileFilter: imageOnlyFilter }).array(
  'proof_files',
  5
);
...
const jobseekerUpload = multer({
  storage: makeStorage(ROLE.JOBSEEKER, 'full_name', 'unknown_jobseeker'),
  fileFilter: imageOnlyFilter,
});
const businessEmployerUpload = multer({
  storage: makeStorage(ROLE.BUSINESS_EMPLOYER, 'business_name', 'unknown_business'),
  fileFilter: imageOnlyFilter,
});
const individualEmployerUpload = multer({
  storage: makeStorage(ROLE.INDIVIDUAL_EMPLOYER, 'full_name', 'unknown_individual'),
  fileFilter: imageOnlyFilter,
});
const manpowerProviderUpload = multer({
  storage: makeStorage(ROLE.MANPOWER_PROVIDER, 'agency_name', 'unknown_agency'),
  fileFilter: imageOnlyFilter,
});
