...
export async function findOrCreateAdmin(connection: PoolConnection, data: AdminData) {
  try {
    const [rows] = await connection.execute<RowDataPacket[]>(SELECT_ADMIN_BY_EMAIL, [data.email]);

    if (rows.length > 0) {
      return { alreadyExists: true, user: rows[0] };
    }

    const [result] = await connection.execute<ResultSetHeader>(INSERT_ADMIN, [
      data.email,
      data.hashedPassword,
      1,
      1,
    ...
    // Log connection errors
      ...
      logger.error('Database connection refused in findOrCreateAdmin', {
        email: data.email,
        error,
      });
    }
