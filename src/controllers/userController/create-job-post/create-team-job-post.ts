...
export const createTeamJobPost = async (
    ...
    try {
        ...
        if (!allowedRoles.includes(role)) {
            ...
            return response.status(403).json({ error: "Forbidden: Only manpower providers can create team job posts." });
        }
...
export function validateTeamJobPostInput(data: TeamJobPostInput): ValidationResult {
    const { worker_category, number_of_workers, location, senior_workers, mid_level_workers, junior_workers, entry_level_workers, team_skills } = data;
    ...
    if (!worker_category || !number_of_workers || !location || !team_skills) {
        return { error: "Please provide worker category, number of workers, location, and team skills." };
    }

    // Ensure numeric fields are not negative
    const numericFields = [number_of_workers, senior_workers, mid_level_workers, junior_workers, entry_level_workers];
...
export async function insertTeamJobPost(connection: PoolConnection, data: TeamJobPostInput): Promise<number> {
    const {
        user_id, worker_category, number_of_workers, location,
